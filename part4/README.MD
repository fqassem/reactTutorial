...
Next, we create two simple sample 'test' APIs - a GET and a POST. If this was a stand-alone server, we would need some [CORS](https://www.w3.org/wiki/CORS_Enabled) configuration. However, we're going to proxy this server through our development server (which hosts our application) to simplify things. Since we'll be using a proxy, there's no need for CORS (unless we want to hit the API server directly, which we don't).
...
Next, in *apiServer.js*, we need to `import bodyParser from 'body-parser';`. Next, to support JSON request bodies, we define `const jsonParser = bodyParser.json();` near the top of our file. This is a parser that Express will use to process JSON request bodies. We'll add the parser as a parameter to the POST call we make for '/signIn'. We'll also need to add it as a parameter to any other POST requests we create. Why not just use a catch-all body parser that handles both JSON and x-www-form-urlencoded bodies? [Security issues, mostly](https://fosterelli.co/dangerous-use-of-express-body-parser.html).
...
Run our application, open up a rest client, and make a POST request to 'localhost:8001/api/signIn' with a `Content-Type: application/json` header and the follwing JSON body:
...
You should receive a 200 OK with a token and the information for user 'tomjones' in the response. Our signIn API is working!
...
This service is simply an abstraction over the `fetch` API. It also provides a way to store the token we recieve from the server. You can store JWTs any number of ways (detailed [here](http://stackoverflow.com/a/40376819)). SessionStorage is a simple to understand and easy to implement way of storing your JWTs and should be the preferred method. However, because we don't have auto-login or store persistence, we'll simply store the token as a static variable in our AuthenticationService. This is so the token is removed from memory upon refresh just like the rest of the Redux state. Of course, this isn't sufficient for a real application, but it's something to keep in mind when you create a production app.
...
What if we add a field to our user object and forget to update our Redux state code? While that's mostly outside of the scope of this tutorial, you can check out this [tutorial](http://blog.isquaredsoftware.com/2016/10/practical-redux-part-1-redux-orm-basics/) for further discussion about normalized data storage in Redux.
...
We have a `componentWillMount` React lifecycle method in this page. It checks if the user has a role, meaning they're signed-in. If that's true, we will forward them to the Home Page. This is so that already signed-in users can't access the sign-in page while they're signed-in.

Notice that we've also added a `componentWillReceiveProps` React function. When the 'signIn' action is dispatched and the reducer modifies the global state, all components that are both connected to the store and are 'listening' for the part of the state that was modified (ie those that called `mapStateToProps`) will receive new props. In *SignIn.js*, we will receive the 'user' state as a prop, which we check in `componentWillReceiveProps`. We make sure that the user's role is now 'USER' or 'ADMIN', indicating they have successfully signed in. If that's the case, we know they are signed in, and we forward them to the *Home* page ('/')!

Using `componentWillReceiveProps` is just one of a few ways we can use to forward a user after a successful sign-in. We could also foward them within the 'signIn' async action. We'll show an example of this in the next part of the tutorial.
...
`{ user.role && <p>Welcome, {user.username}</p>}` is simply shorthand for saying: if a user has a role (they are signed in), then display a 'Welcome, *username*' message to them.

### Configuration updates
Notice that we've added quite a few libraries as 'dependency's. We'll need to add them to *webpack.base.config.js* so that they are correctly bundled with the other vendor code. Let's add Redux, React-Redux, Redux-Thunk, and Redux-Form to our *vendor* bundle. If we don't add them, they will get bundled with the application code, and our caching becomes less efficient!
